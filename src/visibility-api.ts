export function trackUserVisibility(): void {
  localStorage.getItem('lastWindowVisibleTimestamp') ? handleVisibilityChange() : null
...
function handleVisibilityChange(): void {
  ...
  if (document.visibilityState === 'visible') {
    const lastWindowVisibleTimestamp: number = new Date(Number(localStorage.getItem('lastWindowVisibleTimestamp'))).getTime()
    const fiftyHoursAfterTimestamp: number = lastWindowVisibleTimestamp + (2 * 60 * 1000)
